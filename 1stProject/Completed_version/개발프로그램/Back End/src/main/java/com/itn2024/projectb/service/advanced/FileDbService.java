...
public class FileDbService {
    ...
    public void insertFile(FileDb fileDb, String fname, String prid) {
        // 1) UUID 생성 (이미지 파일에 대한 고유 이름 생성, DB에만 사용)
        String uuid = UUID.randomUUID().toString();

        // 2) 프로젝트 내 상대 경로 설정
        String projectDir = System.getProperty("user.dir"); // 프로젝트 경로 가져오기
        String imageDirPath = projectDir + "/src/main/resources/static/images/" + prid; // prid 값으로 서브 폴더 경로 설정
        ...
        // 4) 이미지 파일 저장할 파일 객체 생성 (파일 이름 그대로 사용)
        File destination = new File(directory, fname); // fname을 그대로 사용
        ...
        // 6) 이미지 URL 생성 (UUID는 URL에 사용하지 않음, fname만 사용)
        String url = generateDownloadUrl(fname, prid); // fname을 사용하여 URL 생성

        // 7) FileDb 객체에 UUID와 URL 저장
        fileDb.setUuid(uuid);  // UUID를 DB에 저장
        fileDb.setFileUrl(url); // 생성된 URL을 DB에 저장

        // 8) DB에 저장
        fileDbMapper.insertFile(fileDb);
    }

    public String generateDownloadUrl(String fname, String prid) {
        // 이미지 파일은 static/images/{prid}/ 폴더에 저장되고, 해당 URL을 생성
        return ServletUriComponentsBuilder
                .fromCurrentContextPath()                      // 기본 URL
                .path("/images/")                              // 이미지 경로
                .path(prid + "/")                              // prid 값 추가
                .path(fname)                                   // fname을 파일 이름으로 사용
